...
describe("extractOpenapiData for body 2", () => {
  it("should parse a valid OpenAPI body schema for body 2", async () => {
    ...
    expect(functions[0].function.parameters.properties.parameters.properties).toHaveProperty(
      "stocksTicker"
    )
    expect(
    ...
    expect(
      functions[0].function.parameters.properties.parameters.properties.date.type
    ).toBe("string")
    expect(
      functions[0].function.parameters.properties.parameters.properties.date
