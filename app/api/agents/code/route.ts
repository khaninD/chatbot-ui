/**
 * Code Agent A2A Endpoint
 * Exposes the code specialist agent via A2A protocol
 */
 ...
 * Code agent endpoint following A2A protocol
 */
export async function POST(request: Request) {
  try {
    ...
    console.log(
      `[A2A Code Agent] Request received - thread: ${a2aRequest.thread_id}`
    )
    ...
    // Load MCP tools (code agent needs write/edit/execute tools)
    const mcpTools: unknown[] = []
    const mcpServerUrl = process.env.MCP_SERVER_URL

    if (mcpServerUrl) {
      console.log(`[A2A Code Agent] Loading MCP tools from ${mcpServerUrl}`)
      const mcpServer = mcp({
        url: mcpServerUrl,
      ...
      mcpTools.push(...tools)
      console.log(`[A2A Code Agent] Loaded ${tools.length} MCP tools`)
    }
    ...
    const codeWorkflow = createSingleSpecializedAgent(
      "coder",
      llm,
      mcpTools as any,
      undefined,
    ...
    console.log(`[A2A Code Agent] Query: ${userQuery.substring(0, 100)}...`)

    // Run the code agent
    const result = await codeWorkflow.run(userQuery, {
      chatHistory
    })

    console.log(`[A2A Code Agent] Response generated`)
    ...
    console.error("[A2A Code Agent] Error:", error)

    const errorMessage =
      error instanceof Error ? error.message : "Code agent error"

    return Response.json(createA2AError("unknown", errorMessage, "coder"), {
      status: 500
    })
  }
}
